...
class OrderItem(Base, BaseModelMixin):
    ...
    unit_price_kes_cents: Mapped[int] = mapped_column(Integer, nullable=False)
    subtotal_kes_cents: Mapped[int] = mapped_column(Integer, nullable=False)
    ...
    def unit_price(self) -> float:
        return self.unit_price_kes_cents / 100

    @unit_price.setter
    def unit_price(self, value: float) -> None:
        self.unit_price_kes_cents = int(round(float(value) * 100))

    @property
    def subtotal(self) -> float:
        return self.subtotal_kes_cents / 100

    @subtotal.setter
    def subtotal(self, value: float) -> None:
        self.subtotal_kes_cents = int(round(float(value) * 100))
